...
from ..drawables import STANDARD_VERTEX_ATTR_DTYPES
from .bound import (
    BoundBox,
    BoundCapsule,
    BoundComposite,
...
class ArrayShaderParameter(ShaderParameter):
    ...
    def __hash__(self) -> int:
        values_unpacked = [x for vector in self.values for x in [vector.x, vector.y, vector.z, vector.w]]
...
def get_sorted_layout_names(names):
    order = [
        "Position",
        "BlendWeights",
        "BlendIndices",
    ...
    # filter order to include only present names
    sorted_names = [n for n in order if n in names]
    # append remaining names
    remaining = [n for n in names if n not in sorted_names]
...
class VertexBuffer(ElementTree):
    ...
    VERT_ATTR_DTYPES = STANDARD_VERTEX_ATTR_DTYPES
    ...
    def to_xml(self):
        self.layout = get_sorted_layout_names(self.data.dtype.names)
        element = super().to_xml()
    ...
    def _load_data_from_str(self, _str: str):
        ...
        #     raw_struct_dtype = np.dtype(
        #         [
        #             normal_fmt if attr_name == "Normal" else self.VERT_ATTR_DTYPES[attr_name]
    ...
    def _data_to_str(self):
        layout = self.get_element("layout")
        vert_arr = self.data
        field_names = layout.value

        # if layout.type == "GTAV2":
        #     # Add back the 4th float of Normal element required by FVF GTAV2
        #     new_struct_dtype = np.dtype(
        #         [
        #             (name, vert_arr.dtype.fields[name][0] if name != "Normal" else (np.float32, 4))
        ...
        vert_arr_2d = np.column_stack([vert_arr[name] for name in field_names])
...
class Drawable(ElementTree, AbstractClass):
    ...
    def all_models(self) -> list[DrawableModel]:
        return (
            self.drawable_models_high + self.drawable_models_med + self.drawable_models_low + self.drawable_models_vlow
        )

    ...
    class VertsProperty(ElementProperty):
        ...
        def from_xml(element: ET.Element):
            ...
            if not verts_hex_str:
                raise ValueError(f"Missing verts data on {OccludeModel.VertsProperty.__name__}")
...
class GrassInstance(ElementTree):
    ...
    def __init__(self):
        super().__init__()
        self.position = TextPropertyRequired("Position")
        self.normal_x = ValueProperty("NormalX", 127)
        self.normal_y = ValueProperty("NormalY", 126)
        self.color = TextPropertyRequired("Color")
        self.scale = ValueProperty("Scale", 255)
        self.ao = ValueProperty("Ao", 255)
        self.pad = TextPropertyRequired("Pad")


class GrassInstanceDataList(ListPropertyRequired):
    """List of grass instances within a batch"""
...
class GrassInstanceList(ElementTree):
    """A batch of grass instances with shared archetype and LOD settings"""
    tag_name = "Item"

    def __init__(self):
        super().__init__()
        self.batch_aabb = AABB()
        self.scale_range = VectorProperty("ScaleRange")
        self.archetype_name = TextPropertyRequired("archetypeName")
        self.lod_dist = ValueProperty("lodDist", 120)
        self.lod_fade_start_dist = ValueProperty("LodFadeStartDist", 15)
        self.lod_inst_fade_range = ValueProperty("LodInstFadeRange", 0.75)
        self.orient_to_terrain = ValueProperty("OrientToTerrain", 1)
        self.instance_list = GrassInstanceDataList()


class GrassInstanceListList(ListPropertyRequired):
    """List of grass batches"""
    list_type = GrassInstanceList
...
class PropInstance(ElementTree):
    """Placeholder for prop instance - not yet fully implemented"""
    tag_name = "Item"
...
class PropInstanceListList(ListPropertyRequired):
    """Placeholder for prop instances - not yet implemented"""
    list_type = PropInstance
...
class InstancedData(ElementTree):
    """Container for all instanced data (grass, props, etc.)"""
    tag_name = "instancedData"

    def __init__(self):
        super().__init__()
        self.imap_link = TextPropertyRequired("ImapLink")
        self.prop_instance_list = PropInstanceListList()
        self.grass_instance_list = GrassInstanceListList()
...
class CMapData(ElementTree, AbstractClass):
    ...
    def __init__(self):
        ...
        self.instanced_data = InstancedData()
